...
@dataclass
class OCPDemoEngine:
    base_rate: float = 10.0  # 1 CNY -> 10 token
    ...
    def register_model(
        ...
        self._log(
            ...
            model_id,
            {
                ...
                "price": f"{token_price_per_call:.2f}",
                "quota": str(max_calls_quota),
            },
        )
    ...
    def process_payment_receipt(
        ...
        # Simplified idempotency check: receipt_id + nonce as unique key.
        ...
        self._log(
            ...
            receipt_id,
            {
                ...
                "cny_amount": f"{cny_amount:.2f}",
                "minted_token": f"{minted:.2f}",
            },
        )
        return minted
    ...
    def consume_model(self, user_account_id: str, model_id: str, call_count: int = 1) -> float:
        ...
        if right.disputed:
            self._log(
                "REVENUE_FROZEN_DUE_TO_DISPUTE",
                model_id,
                {"beneficiary": right.beneficiary_account_id, "amount": f"{total_cost:.2f}"},
            )
        else:
